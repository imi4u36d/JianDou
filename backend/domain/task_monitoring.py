...
from backend.shared import map_value, safe_int, string_value
...
def task_monitoring_snapshot(task: Any) -> dict[str, Any]:
    ...
    return {
        "currentStage": first_non_blank(
            string_value(ctx.get("currentStage")),
            string_value(latest_stage_run.get("stageName")),
            string_value(latest_stage_run.get("stage")),
            string_value(active_attempt.get("stageName")),
            string_value(active_attempt.get("resumeFromStage")),
            string_value(latest_trace.get("stage")),
        ),
        "activeWorkerInstanceId": first_non_blank(
            string_value(active_attempt.get("workerInstanceId")),
            string_value(ctx.get("workerInstanceId")),
            string_value(latest_stage_run.get("workerInstanceId")),
            string_value(latest_trace.get("workerInstanceId")),
        ),
        "activeAttemptStatus": string_value(active_attempt.get("status")),
        ...
        "resumeFromStage": first_non_blank(string_value(active_attempt.get("resumeFromStage")), "render"),
        ...
        "latestJoinName": first_non_blank(
            ...
            string_value(map_value(latest_join_output.get("extra")).get("joinName")),
        ),
...
def _active_attempt(task: Any) -> dict[str, Any]:
    ...
    active_attempt_id = string_value(getattr(task, "active_attempt_id", ""))
...
def _planned_clip_count(execution_context: dict[str, Any]) -> int:
    planned = safe_int(execution_context.get("plannedClipCount"), 0)
    if planned > 0:
        return planned
...
def _latest_by(items: list[Any], key: str) -> dict[str, Any]:
    ...
    if not dicts:
        return {}
    if key == "clipIndex":
        return max(dicts, key=lambda item: safe_int(item.get(key), 0), default={})
    return max(dicts, key=lambda item: string_value(item.get(key)), default={})
