...
class FrameResolution:
    def __init__(
        ...
        **legacy_values: Any,
        ...
        self._prompt = _string_value(legacy_values.pop("prompt_value", prompt))
        self._frame_role = _string_value(legacy_values.pop("frame_role_value", frame_role))
        self._source_type = _string_value(legacy_values.pop("source_type_value", source_type))
        self._source_url = _string_value(legacy_values.pop("source_url_value", source_url))
        self._material_url = _string_value(legacy_values.pop("material_url_value", material_url))
        self._remote_url = _string_value(legacy_values.pop("remote_url_value", remote_url))
        self._video_input_url = _string_value(legacy_values.pop("video_input_url_value", video_input_url))
        self._run_id = _string_value(legacy_values.pop("run_id_value", run_id))
...
def build_clip_frame_context(
    ...
    return {
        ...
        "scene": _callable_attr(shot_plan, "scene"),
        "targetDurationSeconds": clip_duration_seconds,
        "startFramePrompt": _first_non_blank(
            start_frame.prompt(),
            _callable_attr(shot_plan, "first_frame_prompt"),
            _callable_attr(shot_plan, "last_frame_prompt"),
        ),
        ...
        "endFramePrompt": _first_non_blank(end_frame.prompt(), _callable_attr(shot_plan, "last_frame_prompt")),
        "endFrameConstraintUrl": end_frame.video_input_url(),
        "endFrameSourceType": end_frame.source_type(),
        "endFrameSourceUrl": end_frame.source_url(),
        "endFrameKeyframeUrl": end_frame.material_url(),
