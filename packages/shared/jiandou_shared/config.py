...
class ModelSettings:
    ...
    @property
    def seedance_video_endpoint(self) -> str:
        provider = self._provider_for_model("seedance-1.5-pro")
        return provider.base_url if provider is not None else ""

    @property
    def seedance_video_task_endpoint(self) -> str:
        provider = self._provider_for_model("seedance-1.5-pro")
        if provider is None:
            return ""
        return str(provider.extras.get("task_base_url") or "").strip()
    ...
    @property
    def seedance_poll_interval_seconds(self) -> int:
        provider = self._provider_for_model("seedance-1.5-pro")
        if provider is None:
            return 8
        return int(provider.extras.get("poll_interval_seconds") or 8)
    ...
    @property
    def seedance_poll_timeout_seconds(self) -> int:
        provider = self._provider_for_model("seedance-1.5-pro")
        if provider is None:
            return 600
        return int(provider.extras.get("poll_timeout_seconds") or 600)
...
def _parse_provider_settings(raw_model: dict[str, Any]) -> dict[str, ProviderSettings]:
    ...
    for key, item in raw_providers.items():
        ...
        provider_name = _required_text(provider_section.get("provider") or key, field=f"model.providers.{key}.provider")
        if provider_name != key:
            raise ValueError(
                f"model.providers.{key}.provider must match provider key '{key}', got '{provider_name}'"
            )
...
def _parse_model_definitions(raw_model: dict[str, Any], providers: dict[str, ProviderSettings]) -> dict[str, ModelDefinition]:
    ...
    for key, item in raw_models.items():
        ...
        provider_name = _required_text(model_section.get("provider"), field=f"model.models.{key}.provider")
        if provider_name not in providers:
            raise ValueError(f"model '{key}' references unknown provider '{provider_name}'")
...
def resolve_text_analysis_target(model: ModelSettings, requested_model: str | None = None) -> RemoteModelTarget:
    ...
    route_mode = definition.provider
...
def load_settings(config_path: str | Path | None = None) -> Settings:
    ...
    prompt_path_value = _pick(prompt, "file", "JIANDOU_PROMPTS_PATH")
    prompt_config_path = (
    ...
    return Settings(
        ...
        config_path=path,
        app=AppSettings(
            name=_required_text(_pick(app, "name", "JIANDOU_APP_NAME"), field="app.name"),
            env=_required_text(_pick(app, "env", "JIANDOU_ENV"), field="app.env"),
            api_host=_required_text(_pick(app, "api_host", "JIANDOU_API_HOST"), field="app.api_host"),
            api_port=_required_int(_pick(app, "api_port", "JIANDOU_API_PORT"), field="app.api_port"),
            web_origin=_required_text(_pick(app, "web_origin", "JIANDOU_WEB_ORIGIN"), field="app.web_origin"),
            execution_mode=_required_text(
                _pick(app, "execution_mode", "JIANDOU_EXECUTION_MODE"),
                field="app.execution_mode",
            ),
        ),
        database=DatabaseSettings(
            url=_required_text(_pick(database, "url", "JIANDOU_DATABASE_URL"), field="database.url"),
            echo=_required_bool(_pick(database, "echo", "JIANDOU_DATABASE_ECHO"), field="database.echo"),
        ),
        redis=RedisSettings(
            url=_required_text(_pick(redis, "url", "JIANDOU_REDIS_URL"), field="redis.url"),
        ),
        storage=StorageSettings(
            root_dir=_required_text(_pick(storage, "root_dir", "JIANDOU_STORAGE_ROOT"), field="storage.root_dir"),
            uploads_dir=_required_text(_pick(storage, "uploads_dir", "JIANDOU_UPLOADS_DIR"), field="storage.uploads_dir"),
            outputs_dir=_required_text(_pick(storage, "outputs_dir", "JIANDOU_OUTPUTS_DIR"), field="storage.outputs_dir"),
            temp_dir=_required_text(_pick(storage, "temp_dir", "JIANDOU_TEMP_DIR"), field="storage.temp_dir"),
            public_base_url=_required_text(
                _pick(storage, "public_base_url", "JIANDOU_PUBLIC_BASE_URL"),
        ...
        model=ModelSettings(
            ...
            aliyun_billing_access_key_id=_optional_text(
                _pick(model, "aliyun_billing_access_key_id", "JIANDOU_ALIYUN_BILLING_ACCESS_KEY_ID")
            )
            or "",
            aliyun_billing_access_key_secret=_optional_text(
                _pick(model, "aliyun_billing_access_key_secret", "JIANDOU_ALIYUN_BILLING_ACCESS_KEY_SECRET")
            )
            or "",
            volcengine_billing_access_key_id=_optional_text(
                _pick(model, "volcengine_billing_access_key_id", "JIANDOU_VOLCENGINE_BILLING_ACCESS_KEY_ID")
            )
            or "",
            volcengine_billing_access_key_secret=_optional_text(
                _pick(model, "volcengine_billing_access_key_secret", "JIANDOU_VOLCENGINE_BILLING_ACCESS_KEY_SECRET")
            )
            or "",
            video_model_usage_quota=_optional_text(
                _pick(model, "video_model_usage_quota", "JIANDOU_VIDEO_MODEL_USAGE_QUOTA")
            )
            or "",
            timeout_seconds=_required_int(_pick(model, "timeout_seconds", "JIANDOU_MODEL_TIMEOUT"), field="model.timeout_seconds"),
            temperature=_required_float(_pick(model, "temperature", "JIANDOU_MODEL_TEMPERATURE"), field="model.temperature"),
            max_tokens=_required_int(_pick(model, "max_tokens", "JIANDOU_MODEL_MAX_TOKENS"), field="model.max_tokens"),
            vision_frame_count=_required_int(
                _pick(model, "vision_frame_count", "JIANDOU_VISION_FRAME_COUNT"),
                field="model.vision_frame_count",
            ),
        ),
        pipeline=PipelineSettings(
            default_aspect_ratio=_required_text(
                _pick(pipeline, "default_aspect_ratio", "JIANDOU_DEFAULT_ASPECT_RATIO"),
                field="pipeline.default_aspect_ratio",
            ),
            max_output_count=_required_int(
                _pick(pipeline, "max_output_count", "JIANDOU_MAX_OUTPUT_COUNT"),
                field="pipeline.max_output_count",
            ),
            max_source_minutes=_required_int(
                _pick(pipeline, "max_source_minutes", "JIANDOU_MAX_SOURCE_MINUTES"),
                field="pipeline.max_source_minutes",
            ),
            default_intro_template=_required_text(
                _pick(pipeline, "default_intro_template", "JIANDOU_DEFAULT_INTRO_TEMPLATE"),
                field="pipeline.default_intro_template",
            ),
            default_outro_template=_required_text(
                _pick(pipeline, "default_outro_template", "JIANDOU_DEFAULT_OUTRO_TEMPLATE"),
