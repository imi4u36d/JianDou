...
from jiandou_ai.providers import ModelGateway, ModelRouter
from jiandou_shared.config import Settings
from jiandou_db.models import MaterialAsset, SystemLog, Task, TaskModelCall, TaskResult, TaskStatusHistory
from jiandou_ai.planner import PlannerChain, parse_transcript_cues
...
from jiandou_shared.utils import extract_llm_text_response, new_id, parse_json_object, truncate_text, utcnow
...
class TaskService:
    ...
    def _load_official_video_model_usage(self, video_models: list[object]) -> dict[str, dict[str, object]]:
        ...
        for model in video_models:
            ...
            if provider in {"seedance", "seedream"}:
                volc_models.append(model_key)
            elif provider in {"wan", "qwen"}:
    ...
    def _build_local_usage_note(self, model_key: str) -> str:
        ...
        if provider in {"seedance", "seedream"} and not self._has_volcengine_billing_credentials():
            return "未配置火山费用中心 AK/SK，当前显示本地累计统计。"
        if provider in {"wan", "qwen"} and not self._has_aliyun_billing_credentials():
    ...
    def _match_volcengine_model_key(self, row: dict[str, object], model_keys: list[str]) -> str:
        ...
        if "seedance-1.5-pro" in model_keys and any(keyword in text for keyword in ("seedance", "火山", "豆包")):
    ...
    def _infer_image_provider(self, model_name: str | None) -> str:
        try:
            ...
            return target.provider or "unknown"
        except Exception:
    ...
    def query_seedance_task_result(self, remote_task_id: str) -> SeedanceTaskQueryResponse:
        normalized_task_id = str(remote_task_id or "").strip()
        if not normalized_task_id:
            raise ValueError("remote task id is required")

        seedance_model = self.settings.model.models.get("seedance-1.5-pro")
        provider = self.settings.model.providers.get(seedance_model.provider) if seedance_model is not None else None
        ...
        if not task_endpoint:
            raise ValueError("seedance video task endpoint is empty")
        if not api_key:
            raise ValueError("seedance api key is empty")
        ...
        try:
            ...
            if detail_text:
                raise RuntimeError(f"seedance task query http error: {exc.code} {detail_text}") from exc
            raise RuntimeError(f"seedance task query http error: {exc.code}") from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(f"seedance task query network error: {exc}") from exc
        except (TimeoutError, socket.timeout) as exc:
            raise RuntimeError(f"seedance task query timeout: {exc}") from exc

        try:
            payload = json.loads(raw_response.decode("utf-8"))
        except Exception as exc:
            raise RuntimeError("seedance task query returned non-json response") from exc
        if not isinstance(payload, dict):
            raise RuntimeError("seedance task query returned non-object payload")
        ...
        return SeedanceTaskQueryResponse(
        ...
        try:
            ...
            if reuse_analysis_result:
                ...
                try:
                    ...
                    raise RuntimeError(analysis_failure_message) from exc
                if analysis_response.status != GenerationRunStatus.SUCCEEDED or analysis_response.resultScript is None:
                    ...
                    raise RuntimeError(analysis_failure_message)
            ...
            requires_reference_image = normalized_video_model == "seedance-1.5-pro"
            if requires_reference_image and not keyframe_output_url:
                raise RuntimeError("seedance-1.5-pro requires reference image url, but keyframe url is empty")
            ...
            for index, clip_prompt in enumerate(clip_prompts):
                ...
                if requires_reference_image and not clip_keyframe_output_url:
                    raise RuntimeError("seedance-1.5-pro requires reference image url, but clip keyframe url is empty")
    ...
    def _resolve_video_reference_image_url(
        ...
        *,
        ...
        normalized_video_model = self.normalize_video_model_key(video_model or "")
        if normalized_video_model == "seedance-1.5-pro":
            if remote_url:
                return remote_url
            return local_url
