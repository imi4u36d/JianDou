...
from jiandou_ai.generation_v2 import PlannerModelGateway
from jiandou_ai.providers import ModelRouter
from jiandou_shared.config import Settings
from jiandou_media.media import detect_scene_changes, sample_audio_peaks, sample_video_frames
from jiandou_shared.schemas import ClipPlan, MediaProbe, TaskSpec
...
class VisionEventAnalyzer:
    ...
    def _build_messages(
        ...
        with tempfile.TemporaryDirectory(prefix="jiandou-vision-") as temp_dir:
            ...
            messages = [
                {"role": "system", "content": self.settings.prompts.planner_shot_analysis_json_only},
    ...
    def _call_model(
        ...
        try:
            body, raw = self.gateway.invoke_vision(
            ...
            if context.trace is not None:
                context.trace(
                    "vision",
                    "vision.request_error",
                    f"视觉模型 {model_name} 请求失败。",
                    {
                        "model": model_name,
                        "source_asset_id": str(source_entry["asset_id"]),
                        "source_file_name": str(source_entry["file_name"]),
...
class FusionPlanner:
    ...
    def _build_prompt(
        ...
        role_header = "你是一个短剧剪辑规划引擎，只输出 JSON，不要解释。"
    ...
    def _call_model(
        ...
        try:
            ...
            if context.trace is not None:
                context.trace(
                    "fusion",
                    "fusion.request_error",
                    f"融合规划模型 {model_name} 请求失败。",
                    {
                        "model": model_name,
                        "error": str(exc),
                    },
