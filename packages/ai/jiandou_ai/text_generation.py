...
from jiandou_ai.generation import ImageGenerationModule, VideoGenerationModule
from jiandou_storage.storage import MediaStorage
from jiandou_shared.utils import (
...
try:
    from jiandou_shared.config import Settings as _SharedSettings  # type: ignore[attr-defined]
except Exception:
    _SharedSettings = Any  # type: ignore[assignment]

try:
    from jiandou_shared.config import resolve_text_analysis_target as _legacy_resolve_text_analysis_target  # type: ignore[attr-defined]
except Exception:
...
try:
    from jiandou_shared.schemas import GenerationTextAnalysisModelInfo as _SchemaGenerationTextAnalysisModelInfo  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerationTextAnalysisModelInfo = None

try:
    from jiandou_shared.schemas import GenerationVideoModelOption as _SchemaGenerationVideoModelOption  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerationVideoModelOption = None

try:
    from jiandou_shared.schemas import GenerationVideoSizeOption as _SchemaGenerationVideoSizeOption  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerationVideoSizeOption = None

try:
    from jiandou_shared.schemas import GenerateTextMediaRequest as _SchemaGenerateTextMediaRequest  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerateTextMediaRequest = None

try:
    from jiandou_shared.schemas import GenerateTextMediaResponse as _SchemaGenerateTextMediaResponse  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerateTextMediaResponse = None

try:
    from jiandou_shared.schemas import ProbeTextAnalysisModelRequest as _SchemaProbeTextAnalysisModelRequest  # type: ignore[attr-defined]
except Exception:
    _SchemaProbeTextAnalysisModelRequest = None

try:
    from jiandou_shared.schemas import ProbeTextAnalysisModelResponse as _SchemaProbeTextAnalysisModelResponse  # type: ignore[attr-defined]
except Exception:
    _SchemaProbeTextAnalysisModelResponse = None

try:
    from jiandou_shared.schemas import GenerateTextScriptRequest as _SchemaGenerateTextScriptRequest  # type: ignore[attr-defined]
except Exception:
    _SchemaGenerateTextScriptRequest = None
...
class _VideoModelSpec:
    ...
    provider_kind: Literal["dashscope", "qwen_vl_workflow", "seedance"] = "dashscope"
...
_VIDEO_MODEL_PROFILES: tuple[_VideoModelProfile, ...] = (
    ...
    _VideoModelProfile(
        ...
        summary="Seedance 1.5 Pro 图生视频模型（异步任务接口）。",
        sizes=(
            _VIDEO_SIZE_720P_16_9,
            _VIDEO_SIZE_720P_9_16,
            _VIDEO_SIZE_1080P_16_9,
...
_VIDEO_MODELS: dict[str, _VideoModelSpec] = {
    ...
    "seedance-1.5-pro": _VideoModelSpec(
...
_VIDEO_MODEL_ALIASES = {
    ...
    "seed-dance-1.5-pro": "seedance-1.5-pro",
    "seedance-1-5-pro": "seedance-1.5-pro",
...
def _video_model_options(
    ...
    for spec in specs or _VIDEO_MODELS.values():
        ...
        elif spec.provider_kind == "seedance":
            provider_label = "火山引擎 Seedance 1.5 Pro"
        ...
        options.append(
            {
                ...
                "provider": (
                    "seedance"
                    if spec.provider_kind == "seedance"
                    else "qwen"
                    if spec.provider_kind == "qwen_vl_workflow"
                    else "wan"
                ),
                "family": "seedance" if spec.provider_kind == "seedance" else "qwen" if spec.provider_kind == "qwen_vl_workflow" else "wan",
...
class TextGenerationEngine:
    ...
    def _build_multipart_form_data(
        ...
        *,
        ...
        boundary = f"----jiandou-{new_id('multipart')}"
        body = bytearray()
    ...
    def _generate_media(
        ...
        if media_type == "video":
            if video_spec and video_spec.provider_kind == "seedance":
                provider_target = self._resolve_video_target(video_spec.name, provider_hint="seedance")
                provider_endpoint = str(provider_target.endpoint or self._resolve_seedance_endpoint()).strip()
                provider_api_key = str(provider_target.api_key or self._resolve_seedance_api_key()).strip()
            else:
        ...
        if media_type == "video" and not task_endpoint_host:
            try:
                if resolved_video_spec and resolved_video_spec.provider_kind == "seedance":
                    task_endpoint_host = urllib.parse.urlparse(self._resolve_seedance_task_endpoint()).netloc or ""
                else:
    ...
    def _generate_video_with_selected_model(
        ...
        *,
        ...
        if spec.provider_kind == "seedance":
            return self._generate_seedance_video(
                request_obj=request_obj,
                profile=profile,
                shaped_prompt=shaped_prompt,
                generation_id=generation_id,
    ...
    def _generate_seedance_video(
        ...
        *,
        ...
        spec = self._resolve_video_model_spec(request_obj)
        target = self._resolve_video_target(spec.name, provider_hint="seedance")
        endpoint = str(target.endpoint or self._resolve_seedance_endpoint()).strip()
        task_endpoint = str(target.task_endpoint or self._resolve_seedance_task_endpoint()).strip()
        api_key = str(target.api_key or self._resolve_seedance_api_key()).strip()
        provider_name = str(target.provider or self.infer_video_provider(spec.name)).strip()
        if not endpoint:
            raise RuntimeError("seedance video endpoint is empty")
        if not task_endpoint:
            raise RuntimeError("seedance video task endpoint is empty")
        if not api_key:
            raise RuntimeError("seedance api key is empty")
        ...
        reference_image_url = self._resolve_seedance_reference_image_url(request_obj)
        if not reference_image_url:
            raise RuntimeError("seedance-1.5-pro requires a reference image url")
        ...
        self._trace_call(
            ...
            event="sent",
            status="ok",
            message="seedance video task submitted",
            details={
                "modelName": spec.name,
        ...
        try:
            ...
            if detail_text:
                raise RuntimeError(f"seedance video task submit http error: {exc.code} {detail_text}") from exc
            raise RuntimeError(f"seedance video task submit http error: {exc.code}") from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(f"seedance video task submit network error: {exc}") from exc
        except (TimeoutError, socket.timeout) as exc:
            raise RuntimeError(f"seedance video task submit timeout: {exc}") from exc

        payload = self._parse_json_bytes(raw_response)
        task_id = self._extract_task_id(payload)
        if not task_id:
            raise RuntimeError("seedance video task response missing task id")
        ...
        self._trace_call(
            ...
            event="created",
            status="ok",
            message="seedance task created",
            details={"taskId": task_id},
        )
        task_marker = f"[seedance_task_id={task_id}]"
        try:
            poll_result = self._poll_seedance_video_task(
                task_id=task_id,
                task_endpoint=task_endpoint,
                api_key=api_key,
                call_chain=call_chain,
            ...
            raise RuntimeError(
                truncate_text(f"{task_marker} seedance task poll failed: {exc}", 700)
                or f"{task_marker} seedance task poll failed"
            ) from exc
        ...
        if not video_url:
            ...
            raise RuntimeError(
                truncate_text(
                    (
                        f"{task_marker} seedance task completed without video_url; "
                ...
                or f"{task_marker} seedance task completed without video_url"
            )
        try:
            ...
            raise RuntimeError(
                truncate_text(f"{task_marker} seedance video download failed: {exc}", 700)
                or f"{task_marker} seedance video download failed"
            ) from exc
    ...
    def _resolve_seedance_reference_image_url(self, request_obj: GenerateTextMediaRequest) -> str:
        extras = getattr(request_obj, "extras", None)
        if not isinstance(extras, dict):
            return ""
    ...
    def _generate_dashscope_video(
        ...
        *,
        ...
        spec = self._resolve_video_model_spec(request_obj)
        provider_hint = "seedance" if spec.provider_kind == "seedance" else "qwen" if spec.provider_kind == "qwen_vl_workflow" else "wan"
    ...
    def _poll_seedance_video_task(
        ...
        *,
        ...
        deadline = time.monotonic() + max(30.0, float(self.settings.model.seedance_poll_timeout_seconds))
        interval = max(1.0, float(self.settings.model.seedance_poll_interval_seconds))
        poll_url = f"{task_endpoint.rstrip('/')}/{urllib.parse.quote(task_id)}"
        attempt = 0
        last_payload: dict[str, Any] = {}
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                if int(exc.code) >= 500:
                    time.sleep(interval)
                    continue
                raise RuntimeError(f"seedance task poll http error: {exc.code}") from exc
            except urllib.error.URLError as exc:
                time.sleep(interval)
                if attempt < 3:
                    continue
                raise RuntimeError(f"seedance task poll network error: {exc}") from exc
            ...
            if task_status in {"FAILED", "FAIL", "CANCELED", "CANCELLED"}:
                message = self._first_str(payload, ("message", "error")) or "seedance task failed"
                raise RuntimeError(message)
            self._trace_call(
                call_chain,
                stage="task_poll",
                event="attempt",
                status="ok",
                message="seedance task polled",
                details={"attempt": attempt, "taskStatus": task_status or "UNKNOWN"},
            )
            time.sleep(interval)
        raise RuntimeError(
            truncate_text(
                f"seedance task poll timeout for task {task_id}; last payload keys={list(last_payload.keys())[:8]}",
                400,
            )
            or f"seedance task poll timeout for task {task_id}"
        )
    ...
    def _resolve_seedance_endpoint(self) -> str:
        resolved = self._resolve_video_target("seedance-1.5-pro", provider_hint="seedance")
        endpoint = str(resolved.endpoint or "").strip()
        if endpoint:
            return endpoint
        endpoint = str(self._model_value("seedance_video_endpoint", default="") or "").strip()
        if endpoint:
            return endpoint
        return ""
    ...
    def _resolve_seedream_image_endpoint(self) -> str:
        resolved = self._resolve_model_target(
            ...
            legacy_default_provider="seedream",
        ...
        if endpoint:
            ...
            return endpoint
        candidates = (
            str(self._model_value("seedance_video_endpoint", default="") or "").strip(),
            str(self._model_value("seedance_video_task_endpoint", default="") or "").strip(),
    ...
    def _resolve_seedance_task_endpoint(self) -> str:
        resolved = self._resolve_video_target("seedance-1.5-pro", provider_hint="seedance")
        endpoint = str(resolved.task_endpoint or "").strip()
        if endpoint:
            return endpoint
        endpoint = str(self._model_value("seedance_video_task_endpoint", default="") or "").strip()
        if endpoint:
            return endpoint
        return self._resolve_seedance_endpoint()

    def _resolve_seedance_api_key(self) -> str:
        resolved = self._resolve_video_target("seedance-1.5-pro", provider_hint="seedance")
        explicit = str(resolved.api_key or "").strip()
        if explicit:
            return explicit
        explicit = str(self._model_value("seedance_api_key", default="") or "").strip()
        if explicit:
            return explicit
    ...
    def _resolve_seedream_api_key(self) -> str:
        target = self._resolve_model_target(
            ...
            legacy_api_key=self._resolve_seedance_api_key(),
        )
        return str(target.api_key or "").strip() or self._resolve_seedance_api_key()
