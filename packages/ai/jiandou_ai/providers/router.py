...
from jiandou_shared.config import ModelDefinition, Settings
...
class ModelRouter:
    ...
    def resolve(
        ...
        *,
        ...
        provider_config = self.settings.model.providers.get(model.provider)
        if provider_config is None:
            raise RuntimeError(f"provider '{model.provider}' is not configured for model '{selected}'")
        return ResolvedModelTarget(model=model, provider=model.provider, provider_config=provider_config)
