...
def upgrade() -> None:
    ...
    with op.batch_alter_table("sys_user_model_credential") as batch_op:
        if "base_url" not in columns:
            batch_op.alter_column("base_url", existing_type=sa.String(length=1024), server_default=None)
        if "task_base_url" not in columns:
            batch_op.alter_column("task_base_url", existing_type=sa.String(length=1024), server_default=None)
        if "extras_json" not in columns:
            batch_op.alter_column("extras_json", existing_type=sa.String(length=2048), server_default=None)
