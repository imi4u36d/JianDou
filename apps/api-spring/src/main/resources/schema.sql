...
CREATE TABLE IF NOT EXISTS `biz_material_assets` (
  ...
  `owner_user_id` bigint unsigned DEFAULT NULL COMMENT '归属用户ID',
  `task_id` varchar(64) NOT NULL DEFAULT '' COMMENT '关联任务ID',
  `workflow_id` varchar(64) NOT NULL DEFAULT '' COMMENT '关联工作流ID',
  ...
  `stage_type` varchar(32) NOT NULL DEFAULT '' COMMENT '阶段类型',
  `clip_index` int NOT NULL DEFAULT '0' COMMENT '镜头序号',
  `version_no` int NOT NULL DEFAULT '0' COMMENT '版本号',
  `selected_for_next` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否被选为继续依据',
  `user_rating` int DEFAULT NULL COMMENT '用户评分',
  `rating_note` varchar(1000) NOT NULL DEFAULT '' COMMENT '评分备注',
  ...
  KEY `idx_material_assets_owner_time` (`owner_user_id`, `create_time`),
  KEY `idx_material_assets_workflow_stage_clip` (`workflow_id`, `stage_type`, `clip_index`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='素材表';

CREATE TABLE IF NOT EXISTS `biz_stage_workflows` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `workflow_id` varchar(64) NOT NULL COMMENT '工作流业务ID',
  `owner_user_id` bigint unsigned DEFAULT NULL COMMENT '归属用户ID',
  `title` varchar(255) NOT NULL DEFAULT '' COMMENT '工作流标题',
  `transcript_text` longtext COMMENT '正文内容',
  `global_prompt` longtext COMMENT '全局提示词',
  ...
  `task_seed` int DEFAULT NULL COMMENT '种子',
  `min_duration_seconds` int unsigned NOT NULL DEFAULT '0' COMMENT '最小时长秒',
  `max_duration_seconds` int unsigned NOT NULL DEFAULT '0' COMMENT '最大时长秒',
  `status` varchar(32) NOT NULL DEFAULT 'DRAFT' COMMENT '工作流状态',
  `current_stage` varchar(32) NOT NULL DEFAULT 'storyboard' COMMENT '当前阶段',
  `selected_storyboard_version_id` varchar(64) NOT NULL DEFAULT '' COMMENT '当前选中的分镜版本',
  `final_join_asset_id` varchar(64) NOT NULL DEFAULT '' COMMENT '最终拼接素材ID',
  `effect_rating` int DEFAULT NULL COMMENT '最终评分',
  `effect_rating_note` varchar(1000) NOT NULL DEFAULT '' COMMENT '最终评分备注',
  `rated_at` datetime DEFAULT NULL COMMENT '评分时间',
  `metadata_json` json NOT NULL COMMENT '附加元数据',
  `timezone_offset_minutes` int NOT NULL DEFAULT '480' COMMENT '业务时区偏移分钟',
  `is_deleted` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '逻辑删除',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_stage_workflow_id` (`workflow_id`),
  KEY `idx_stage_workflows_owner_time` (`owner_user_id`, `create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='阶段化工作流表';

CREATE TABLE IF NOT EXISTS `biz_stage_versions` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `stage_version_id` varchar(64) NOT NULL COMMENT '阶段版本业务ID',
  `workflow_id` varchar(64) NOT NULL DEFAULT '' COMMENT '工作流业务ID',
  `owner_user_id` bigint unsigned DEFAULT NULL COMMENT '归属用户ID',
  `stage_type` varchar(32) NOT NULL DEFAULT '' COMMENT '阶段类型',
  `clip_index` int NOT NULL DEFAULT '0' COMMENT '镜头序号',
  `version_no` int NOT NULL DEFAULT '1' COMMENT '版本号',
  `title` varchar(255) NOT NULL DEFAULT '' COMMENT '标题',
  `status` varchar(32) NOT NULL DEFAULT 'SUCCEEDED' COMMENT '状态',
  `selected` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否当前选中',
  `rating` int DEFAULT NULL COMMENT '版本评分',
  `rating_note` varchar(1000) NOT NULL DEFAULT '' COMMENT '评分备注',
  `rated_at` datetime DEFAULT NULL COMMENT '评分时间',
  `parent_version_id` varchar(64) NOT NULL DEFAULT '' COMMENT '父版本ID',
  `source_material_asset_id` varchar(64) NOT NULL DEFAULT '' COMMENT '来源素材ID',
  `material_asset_id` varchar(64) NOT NULL DEFAULT '' COMMENT '当前素材ID',
  `preview_url` text NOT NULL COMMENT '预览地址',
  `download_url` text NOT NULL COMMENT '下载地址',
  `input_summary_json` json NOT NULL COMMENT '输入摘要',
  `output_summary_json` json NOT NULL COMMENT '输出摘要',
  `model_call_summary_json` json NOT NULL COMMENT '模型调用摘要',
  `timezone_offset_minutes` int NOT NULL DEFAULT '480' COMMENT '业务时区偏移分钟',
  `is_deleted` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '逻辑删除',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_stage_version_id` (`stage_version_id`),
  KEY `idx_stage_versions_workflow_stage_clip` (`workflow_id`, `stage_type`, `clip_index`, `version_no`),
  KEY `idx_stage_versions_material_asset` (`material_asset_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='阶段版本表';

CREATE TABLE IF NOT EXISTS `biz_material_asset_tags` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `asset_tag_id` varchar(64) NOT NULL COMMENT '标签业务ID',
  `material_asset_id` varchar(64) NOT NULL DEFAULT '' COMMENT '素材业务ID',
  `tag_type` varchar(16) NOT NULL DEFAULT 'system' COMMENT '标签类型',
  `tag_key` varchar(64) NOT NULL DEFAULT '' COMMENT '标签键',
  `tag_value` varchar(128) NOT NULL DEFAULT '' COMMENT '标签值',
  `is_deleted` tinyint unsigned NOT NULL DEFAULT '0' COMMENT '逻辑删除',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_asset_tag_id` (`asset_tag_id`),
  KEY `idx_asset_tags_asset_key` (`material_asset_id`, `tag_key`, `tag_value`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='素材标签表';
