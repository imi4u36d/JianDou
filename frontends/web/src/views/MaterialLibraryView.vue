  ...
  <section class="material-view">
    <header class="material-topbar">
      <nav class="material-tabs" aria-label="素材分类">
        <button v-for="tab in libraryTabs" :key="tab.key" type="button" class="material-tab" :class="{ 'material-tab-active': activeLibraryTab === tab.key }" @click="selectLibraryTab(tab.key)">
          {{ tab.label }}
        </button>
      </nav>
      ...
      <div class="material-topbar__tools liquid-glass">
        ...
        <button class="material-toolbar-link" type="button" :class="{ 'material-toolbar-link-active': advancedFiltersOpen || activeFilterCount > 0 }" aria-label="筛选素材" title="筛选" @click="advancedFiltersOpen = !advancedFiltersOpen">
    ...
    <section class="material-favorite-folders" aria-label="收藏夹">
      <div class="material-favorite-folders__head">
        <button class="jd-button jd-button--ghost jd-button--xs material-favorite-folders__add" type="button" @click="openFavoriteDialog()">
          <IconPlus size="xs" />
          新建收藏夹
        </button>
      </div>
      <div class="material-favorite-folders__list">
        <button v-for="folder in favoriteFolders" :key="folder.id" type="button" class="material-favorite-folder" :class="{ 'material-favorite-folder-active': activeFavoriteFolderId === folder.id }" @click="selectFavoriteFolder(folder.id)">
      ...
      <div class="material-favorite-folders__actions">
        <button class="jd-button jd-button--ghost jd-button--xs material-favorite-folders__batch" type="button" :class="{ 'material-favorite-folders__batch-active': batchMode }" :disabled="!canUseBatchMode" @click="toggleBatchMode">
          <IconCheck size="xs" />
          批量操作
        </button>
    ...
    >
      <template v-if="previewAsset" #actions>
        ...
        <button v-if="isAssetShareable(previewAsset)" type="button" class="jd-button jd-button--sm" :disabled="sharingAssetId === previewAsset.id" aria-label="分享预览素材" @click="openMaterialShareConfirm(previewAsset)">
          <IconShare size="xs" />
    ...
    <Teleport to="body">
      <Transition name="material-favorite-dialog-fade">
        <div v-if="renameDialog.open" class="material-rename-dialog" role="dialog" aria-modal="true" aria-labelledby="material-rename-dialog-title" @click.self="closeRenameDialog" @keydown.esc.stop.prevent="closeRenameDialog">
          <form class="material-rename-dialog__panel" @submit.prevent="commitAssetRename">
            ...
            <label class="material-rename-dialog__field">
              <span>名称</span>
              <input ref="renameInputRef" v-model="renameDialog.title" type="text" maxlength="80" placeholder="输入素材名称" @keydown.stop />
            </label>
...
import { IconCheck, IconClose, IconDelete, IconHeart, IconLoading, IconPlus, IconSearch, IconSettings, IconShare } from "@/components/icons";
...
const { previewAsset, previewImageLoadFailed, previewDialog, canPreviewPrevious, canPreviewNext, openAssetPreview, closePreviewDialog, syncPreviewAsset, navigatePreview } = useMaterialPreview({
  displayedAssets,
  activeFavoriteFolderId,
  hasMoreAssets,
  loadMoreAssets,
...
const favoriteDialogActiveFolderIds = computed(() => favoriteFolders.value.filter((folder) => isFavoriteDialogFolderActive(folder.id)).map((folder) => folder.id));
</script>
